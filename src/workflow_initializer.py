"""
Workflow Initializer Lambda Function with Compatibility Wrapper
Fetches organization mappings and health events, then prepares account processing tasks
Also maintains backward compatibility for manual invocations
"""
import boto3
import json
import os
...
def lambda_handler(event, context):
    """
    Main handler that routes between Step Functions workflow and compatibility mode
    """
    try:
        # Debug logging
        ...
        # Check if this is a compatibility/manual invocation
        source = event.get('source', '')
        detail_type = event.get('detail-type', '')
        
        # Handle scheduled events that should use Step Functions
        if source == 'aws.events' and 'Scheduled Event' in detail_type:
            print("Scheduled execution detected - redirecting to Step Functions")
            return {
                'statusCode': 200,
                'body': json.dumps({
        ...
        # Check if this is clearly a compatibility/manual invocation (API Gateway or manual test)
        is_api_gateway = event.get('httpMethod') or event.get('requestContext')
        is_manual_test = source == 'manual' or 'test' in event.get('detail-type', '').lower()
        
        if is_api_gateway or is_manual_test:
            print("Compatibility/manual invocation detected")
        ...
        # Default to Step Functions mode (primary use case)
        print("Step Functions workflow execution detected (default mode)")
        return workflow_initializer_handler(event, context)
        
    except Exception as e:
        print(f"Error in workflow initializer: {str(e)}")
        traceback.print_exc()
        
        # Return appropriate error format based on invocation type
        is_api_gateway = event.get('httpMethod') or event.get('requestContext')
        is_scheduled_event = event.get('source') == 'aws.events'
        
        if is_api_gateway or is_scheduled_event:
            return {
                'statusCode': 500,
                'body': json.dumps({
                    'error': str(e),
            ...
            # Re-raise for Step Functions (so it can handle retries)
            raise


def workflow_initializer_handler(event, context):
    """
